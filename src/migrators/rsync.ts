...
export const rsyncMigrator: Migrator = {
	...
	async validate(
		...
		if (sourceCheck.code !== 0)
			errors.push("rsync not installed on source server");

		const targetCheck = await context.target.exec("which rsync");
		if (targetCheck.code !== 0)
			errors.push("rsync not installed on target server");
	...
	async estimate(step: Step, context: MigrationContext): Promise<TimeEstimate> {
		const totalBytes =
			context.plan.services.flatMap((s) => s.volumes).length * 1000000000; // rough estimate
