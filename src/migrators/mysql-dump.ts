...
export const mysqlDumpMigrator: Migrator = {
	...
	async execute(step: Step, context: MigrationContext): Promise<StepResult> {
		...
		const dumpResult = await context.source.exec(
			`docker compose -f ${composePath} exec -T ${service} mysqldump --all-databases -u root -p$MYSQL_ROOT_PASSWORD > ${DUMP_PATH}`,
		);
		...
		context.onLog(
			`Database dump complete: ${(sizeBytes / 1024 / 1024).toFixed(1)} MB`,
		);
