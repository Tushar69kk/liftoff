...
import { Executor, type ExecutionResult } from "../../src/executor/index";
import { MigratorRegistry } from "../../src/migrators/registry";
import { MockSshClient } from "../helpers/mock-ssh";
import type { MigrationPlan, Migrator, Step, MigrationContext, ValidationResult, StepResult, TimeEstimate } from "../../src/types";
...
function makeMockMigrator(type: Step["type"], shouldFail = false): Migrator {
  return {
    ...
    async execute(step: Step, context: MigrationContext): Promise<StepResult> {
      ...
      if (shouldFail) {
        return { success: false, error: "Intentional test failure", duration: 10 };
      }
