...
describe("SshConnection", () => {
	...
	test("wrapWithSudo prepends sudo when needed", () => {
		...
		expect(SshConnection.wrapWithSudo("docker ps", "sudo_nopasswd")).toBe(
			"sudo docker ps",
		);
		expect(
			SshConnection.wrapWithSudo("docker ps", "sudo_passwd", "mypass"),
		).toBe("echo 'mypass' | sudo -S docker ps");
		expect(SshConnection.wrapWithSudo("docker ps", "docker_group")).toBe(
			"docker ps",
		);
	});
});
