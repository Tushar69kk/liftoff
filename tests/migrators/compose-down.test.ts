...
describe("composeDownMigrator", () => {
  test("runs docker compose down on source", async () => {
    const source = new MockSshClient(() => ({ stdout: "", stderr: "", code: 0 }));
    const result = await composeDownMigrator.execute(
      { name: "stop", type: "compose_down" },
      makeContext(source),
    );
    expect(result.success).toBe(true);
    expect(source.commands.some((c) => c.includes("docker compose") && c.includes("down"))).toBe(true);
  });

  test("uses correct compose file path", async () => {
    const source = new MockSshClient(() => ({ stdout: "", stderr: "", code: 0 }));
    await composeDownMigrator.execute(
      { name: "stop", type: "compose_down" },
      makeContext(source),
    );
