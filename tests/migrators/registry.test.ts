...
import type { Migrator, Step, MigrationContext, ValidationResult, StepResult, TimeEstimate } from "../../src/types";
...
describe("MigratorRegistry", () => {
  ...
  test("throws on unregistered type", () => {
    const registry = new MigratorRegistry();
    expect(() => registry.resolve("rsync")).toThrow(/no migrator registered.*rsync/i);
  });
  ...
  test("prevents duplicate registration", () => {
    const registry = new MigratorRegistry();
    registry.register(makeFakeMigrator("rsync"));
    expect(() => registry.register(makeFakeMigrator("rsync"))).toThrow(/already registered/i);
  });
});
