...
describe("detectDatabases", () => {
  ...
  test("detects PostgreSQL from bitnami image", () => {
    const services: Service[] = [{ name: "db", image: "bitnami/postgresql:15.4", volumes: [] }];
    const dbs = detectDatabases(services);
    expect(dbs).toHaveLength(1);
    expect(dbs[0].type).toBe("postgres");
  ...
  test("handles image without version tag", () => {
    const services: Service[] = [{ name: "db", image: "postgres", volumes: [] }];
    const dbs = detectDatabases(services);
    expect(dbs).toHaveLength(1);
  ...
  test("detects MariaDB as mysql type", () => {
    const services: Service[] = [{ name: "db", image: "mariadb:11", volumes: [] }];
    const dbs = detectDatabases(services);
    expect(dbs).toHaveLength(1);
    expect(dbs[0].type).toBe("mysql");
  ...
  test("detects MySQL from bitnami image", () => {
    const services: Service[] = [{ name: "db", image: "bitnami/mysql:8.0", volumes: [] }];
    const dbs = detectDatabases(services);
    expect(dbs).toHaveLength(1);
    expect(dbs[0].type).toBe("mysql");
  ...
  test("detects MariaDB from bitnami image", () => {
    const services: Service[] = [{ name: "db", image: "bitnami/mariadb:11.2", volumes: [] }];
    const dbs = detectDatabases(services);
    expect(dbs).toHaveLength(1);
    expect(dbs[0].type).toBe("mysql");
  ...
  test("detects Redis from image name", () => {
    const services: Service[] = [{ name: "cache", image: "redis:7-alpine", volumes: [] }];
    const dbs = detectDatabases(services);
    expect(dbs).toHaveLength(1);
  ...
  test("detects Redis from bitnami image", () => {
    const services: Service[] = [{ name: "cache", image: "bitnami/redis:7.2", volumes: [] }];
    const dbs = detectDatabases(services);
    expect(dbs).toHaveLength(1);
    expect(dbs[0].type).toBe("redis");
  });
  ...
  test("detects Valkey as redis type", () => {
    const services: Service[] = [{ name: "cache", image: "valkey/valkey:7.2", volumes: [] }];
    const dbs = detectDatabases(services);
    expect(dbs).toHaveLength(1);
    expect(dbs[0].type).toBe("redis");
  });
  ...
  test("detects MongoDB from image name", () => {
    const services: Service[] = [{ name: "mongo-db", image: "mongo:7", volumes: [] }];
    const dbs = detectDatabases(services);
    expect(dbs).toHaveLength(1);
  ...
  test("detects MongoDB from bitnami image", () => {
    const services: Service[] = [{ name: "mongo-db", image: "bitnami/mongodb:7.0", volumes: [] }];
    const dbs = detectDatabases(services);
    expect(dbs).toHaveLength(1);
    expect(dbs[0].type).toBe("mongo");
  });
  ...
  test("detects mixed database stack", () => {
    ...
    expect(dbs.map((d) => d.type).sort()).toEqual(["mongo", "mysql", "postgres", "redis"]);
  });
});
